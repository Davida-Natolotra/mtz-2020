...
class factureForm(ModelForm):
    Facture_Date_Facture = forms.DateField(
        widget=forms.DateInput(attrs={'type': 'date', 'class': 'form-control w3-margin-bottom', 'max': 'today'}),
        required=False)

    Facture_OT_Honoraire = Thousands(label='OT - Honoraire', max_digits=20, decimal_places=2, widget=forms.TextInput())

    Facture_Autres_Montant = Thousands(label='Autres montant', max_digits=20, decimal_places=2,
    ...
    Facture_Surestaries_Montant = Thousands(label='Surestaries', max_digits=20, decimal_places=2,
                                            widget=forms.TextInput())

    Facture_Debarquement = Thousands(label='Debarquement', max_digits=20, decimal_places=2, widget=forms.TextInput())

    Facture_Magasinage_Montant = Thousands(label='Magasinage', max_digits=20, decimal_places=2,
                                           widget=forms.TextInput())

    Facture_Droit_Compromis = Thousands(label='Droit compromis', max_digits=20, decimal_places=2,
                                        widget=forms.TextInput())
    ...
    Facture_Immobilisation = Thousands(label='Immobilisation', max_digits=20, decimal_places=2,
                                       widget=forms.TextInput())
    ...
    class Meta:
        model = Operation
        fields = [
            'Facture_OT_Honoraire', 'Facture_Autres_Montant', 'Facture_BAD_Montant', 'Facture_Overstay_Montant',
            'Facture_Surestaries_Montant', 'Facture_Debarquement', 'Facture_Magasinage_Montant',
            'Facture_Droit_Compromis', 'Facture_Amende_Montant', 'Facture_OV_Montant', 'Facture_OV_Docker',
            'Facture_Montant_Fret', 'Facture_Immobilisation', 'Facture_Num_Facture', 'Facture_Nom_Client',
            'Facture_Adresse_Client', 'Facture_Contact_Client', 'Facture_email_client', 'Facture_Date_Facture',
            'Facture_SousTotal', 'Tax', 'TotalFacture'
        ]
...
class factureMotoForm(ModelForm):
    ...
    class Meta:
        model = Moto
        fields = [
            'nom_client_2', 'CIN_Num_Client_2', 'adresse_client_2', 'tel_client_2', 'volume_moteur', 'PU_HT', 'TVA',
            'PT', 'montant_lettre', 'PV', 'date_vente', 'Ref', 'num_sur_facture'
        ]
