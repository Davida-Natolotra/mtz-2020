...
def Chart_Year_Trimestre_API(request):
    ...
    Trimestriel = {}
    Trim_Name = ["Janvier - Fevrier - Mars", "Avril - Mai - Juin",
...
def Chart_Year_Semestre_API(request):
    ...
    taona = dt.strptime(yearChange, "%Y")
    Semestriel = {}
    Sem_Name = ["Janvier - Fevrier - Mars - Avril - Mai - Juin",
                "Juillet - Aout - Septembre - Octobre - Novembre - Decembre"]
    for t in range(2):
        counter = 0
        for p in range(6):
...
@sync_to_async()
@api_view(['GET'])
def check_num_moteur_unique(request, num_moteur=None):
    check = Moto.objects.filter(num_moteur=num_moteur).count()
    response: Boolean
    if check == 0:
        response = True
    else:
